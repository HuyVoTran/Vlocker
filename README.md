...
git add .
git commit -m "Update payment page, add SSE for locker updates, and optimize dashboard performance"
git push
...
## Cập nhật gần đây
- **Thanh toán trả tủ trong MyLocker:** Thanh toán trực tiếp trong MyLocker với UI dạng modal/card.
- **Tích hợp cổng thanh toán:** Hỗ trợ MoMo, PayPal và VNPay.
- **Chế độ demo thanh toán:** Tùy chọn mô phỏng thanh toán bằng `NEXT_PUBLIC_PAYMENT_MODE=demo`.
- **Realtime trạng thái tủ:** Đồng bộ trạng thái tủ giữa cư dân và quản lý qua SSE (`/api/lockers/stream`).
...
7.  **Realtime Updates:**
  -   **Cơ chế:** Server-Sent Events (SSE) qua endpoint `/api/lockers/stream`.
  -   **Mục đích:** Đồng bộ trạng thái tủ theo thời gian thực giữa cư dân và quản lý (đăng ký, hủy, cập nhật trạng thái...).
